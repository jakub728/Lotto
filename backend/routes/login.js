...
export const cookieOptions = {
  ...
  secure: isProduction,
  sameSite: "lax",
  domain: isProduction ? ".lotto-gen.pl" : undefined,
};
...
router.post("/user/login", async (req, res, next) => {
  try {
    ...
    if (!findUser) {
      return next({ status: 401, message: "Wrong email or password" });
    }

    if (!findUser.verified) {
      return next({ status: 403, message: "Please verify your email before" });
    }
